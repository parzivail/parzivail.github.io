...
On this unit, the lens is nearly 40 years old, and the coatings are breaking down. I haven't yet calibrated it, but I plan to: I'm sure even if it was calibrated when it was put into storage before I got it, the capacitors alone would have de-rated enough to need compensation in the last 30 or so years. As noted in the appendix, the camera is currently without a real IR-cut filter. The image of a cactus below is, of course, also routed through a cheap USB composite capture device.

TODO: retake with IR-cut filter
...
Among the modules connected to the backplane is one called the EN-39. From the block diagram, it's in charge of RGB buffering, [YIQ](https://en.wikipedia.org/wiki/YIQ) matrix amplification, [zebra stripes](https://en.wikipedia.org/wiki/Zebra_patterning), and composite/[S-Video](https://en.wikipedia.org/wiki/S-Video) encoding. Basically, this module is entirely responsible for analog video output.
...
For now I routed the HDMI cable through the battery compartment. The MS9828 correctly sorts out the composite sync signal from the backplane and sorts the pixels into the right fields, which is great. Finding a monitor or even TV around that supported the somewhat oddball 480i resolution was a challenge, and in the end I used a Blackmagic DeckLink Mini capture card, which didn't list that format as supported in the manual, but seems to record it fine. The resulting video might not be broadcast-quality by today's standards but I think it's a noticable improvement over composite or even S-Video for bright, noisy scenes.
...
All filters were measured with a Shimadzu UV-160U (hopefully the subject of a future post!), and the thermal printouts scanned.
